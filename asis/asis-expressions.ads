...
--  class-wide type or an anonymous type.
--
--  --|AN Application Note:
--  --|AN
...
--  wording below may need revision):
...
--  of the argument expression type. This allows to handle the case when the
--  type of the argument expression is anonymous.
--
--  This query returns Nil_Element if and only if the argument Expression does
--  not have type in Ada sense.
--
--  Appropriate Element_Kinds:
--       An_Expression
--
...
--
--  At the moment the query is under development, so think twice before using
--  this query in your tool.

------------------------------------------------------------------------------
--  |D2005 end
--  |A2005 end

--  --|ER A_Box_Expression   -       No child elements
...
--
--  - an identificator or an operator symbol that is specific to SPARK 2014
--    Ada extension;
--
--  - an identifier or an operator symbol that is repeated after a final END
--    of a declaration or a loop or a block name that is repeated at the end of
--    a statement;
--
--  Raises ASIS_Inappropriate_Element, with a Status of Value_Error, if passed
...
--
--  |A2012 start
--  ASIS classifies the Ada 2012 user-defined (generalized) indexing as
--  An_Indexed_Component. According to the Ada 2012, a generalized indexing
--  has not a list of index expressions, but actual_parameter_part as its
--  subcomponents (that is, it can have the form as 'M (A, B)' as well as
--  'M (X => A, Y => B)'. So, if the argument of the query
--  Is_Generalized_Indexing then the result list may contain
--  A_Parameter_Association elements (for position associations)
--  |A2012 end
--
--  Appropriate Expression_Kinds:
...
--
--  If Association element is from a normalized list, three cases are possible:
--
--  * an aggregate explicitly specifies the value for the corresponding
--    record component, in this case the result Is_Equal to the
--    Component_Expression result obtained for this record component from
--    non-normalized association
--
--  * an aggregate does not specify the value for the corresponding record
--    component (in this case the aggregate contains a box), but the
--    A_Component_Declaration for the corresponding record component provides
--    a default initialization expression for this component. In this case
--    the result Is_Equal to the initialization_Expression from
--    A_Component_Declaration
--
--  * an aggregate does not specify the value for the corresponding record
--    component (in this case the aggregate contains a box), and the
--    A_Component_Declaration for the corresponding record component does not
--    provide a default initialization expression for this component. In this
--    case the result of this query is Nil_Element.
--
--  Normalized lists contain artificial ASIS An_Association elements that
--  provide one formal A_Defining_Name => An_Expression pair per
--  association.  These artificial associations are Is_Normalized.  Their
...
--
--  --|AN Application Note:
--  --|AN See the Application Note for
--  --|AN Asis.Elements.Pragma_Argument_Associations for the argument of the
--  --|AN SPARK-specific pragma Depends
--
...
--    represents the name or the expression that is explicitly given in the
--    source, the Enclosing_Element of An_Expression is not the argument
...
--
--  |A2012 start
--  ASIS classifies a user-defined (generalized) indexing as
--  An_Indexed_Component, but actually this is a function call. So
--  Corresponding_Called_Function can be applied for An_Indexed_Component
--  Element in case if it Is_Generalized_Indexing, and the result should be
--  the function that is cassed at the place of the argument feneralized
--  indexing.
--  |A2012 end
--
--  Appropriate Expression_Kinds:
--       A_Function_Call
--       An_Indexed_Component (Is_Generalized_Indexing)  --ASIS 2012
--
--  Returns Declaration_Kinds:
--       Not_A_Declaration
--       A_Function_Declaration
--       An_Expression_Function_Declaration --ASIS 2012
...
--  Is the right side of this assignment statement (in the instantiation!) a
...
-----------------------------------------------------
   ...
   --  Expression  - Specifies the expression to query
   --
   --  Returns True if the argument is a user-defining (generalized) indexing.
   --  ASIS classifies generalized indexing as An_Indexed_Component.
   --
   --  Returns False for any unexpected Element.
   --  Expected Expression_Kinds:
   --       An_Indexed_Component
