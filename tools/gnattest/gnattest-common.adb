   ...
   begin
      ...
      case Declaration_Kind (Subp) is
         ...
         when others =>
            ...
            Trace (Me_Hash, To_String (Element_Image (Subp)));
            Trace (Me_Hash, To_String (Debug_Image (Subp)));
            return "";
      end case;
      ...
      if
        ...
        and then Definition_Kind (Primitive_Owner (Subp)) /=
          A_Private_Type_Definition
      then
         Tagged_Rec := Enclosing_Element (Primitive_Owner (Subp));
      end if;
         ...
         case Definition_Kind (Param) is
            ...
            when Not_A_Definition =>
               ...
               then
                  ...
                  if not Is_Nil (Root_Ignore) then
                     if
                       (Has_Controlling_Result_W (Subp) or else N_Controlling)
                       and then Is_Equal
                         (Root_Ignore,
                          Root_Type_Declaration (Param))
                     then
