               ...
               then
                  ...
                  Test_Routine.Tested_Sloc := new String'
                    (Base_Name (Unit_Im.all)
                     & ":"
                     & Trim (Integer'Image (First_Line_Number (ISubp)), Both)
                     & ":"
                     & Trim (Integer'Image (First_Column_Number (ISubp)), Both)
                     & ": inherited at "
                     & Base_Name
                       (To_String
                          (Text_Name
                             (Enclosing_Compilation_Unit (Original_Type))))
                     & ":"
                     & Trim
            ...
            when A_Procedure_Declaration            |
               ...
               then
                  if Type_Found then
                     ...
                     Gather_Test_Cases (Subp, Test_Routine_Wrapper);
                  end if;
               end if;
   ...
   if Stub_Mode_ON then
      ...
      begin
         ...
         then
            ...
            declare
               Body_Clause_List : constant Asis.Context_Clause_List :=
                 Context_Clause_Elements (Body_Unit);
            begin
               for I in Body_Clause_List'Range loop
                  if
                    Clause_Kind (Body_Clause_List (I)) = A_With_Clause
                    and then Trait_Kind (Body_Clause_List (I)) /=
                      A_Limited_Trait
                  then
                     declare
                        With_Names : constant Asis.Name_List :=
                          Clause_Names (Body_Clause_List (I));
                        First_Idx : constant Integer := With_Names'First;
