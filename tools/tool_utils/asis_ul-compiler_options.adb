...
package body ASIS_UL.Compiler_Options is
   ...
   begin
      ...
      Initialize (Env);
      Env.Set_Path_From_Gnatls (Detect_Target & "gnatls --RTS=" & RTS, Tmp);
      ...
      for J in Dirs'Range loop
         ...
         if Idx /= 0 then
            ...
            begin
               return Trim (Result (F_Idx .. Idx - 2), Both);
            end;
         end if;
      end loop;
      ...
      loop
         ...
         is
            ...
            when 'I' | 'g' =>
               ...
               elsif Full_Switch (Parser => Parser) = "gnatD"
                 or else
                     Full_Switch (Parser => Parser) = "gnatQ"
                 or else
                     Full_Switch (Parser => Parser) = "gnatG"
                 or else
                     Full_Switch (Parser => Parser) = "gnatR"
                 or else
                     Full_Switch (Parser => Parser) = "gnats"
                 or else
                     Full_Switch (Parser => Parser) = "gnatd"
               then
                  null;
   ...
   procedure Store_Option (Switch : String) is
      First_Idx : constant Positive := Switch'First;
   begin
      --  Check for an --RTS option with a short name of RTS. This is temporary
      --  fix for NA20-017

      if Switch'Length >= 7
        and then
         Switch (First_Idx .. First_Idx + 5) = "--RTS="
        and then
         Index (Switch, (1 => Directory_Separator)) = 0
      then
         Compiler_Switches.Append
           (new String'("--RTS=" &
            Get_Full_Path_To_RTS
              (Trim (Switch (First_Idx + 6 .. Switch'Last), Both))));
      else
