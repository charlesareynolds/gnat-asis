...
--     First_Process .. Max_Processes;
   ...
   begin
      ...
      Tree_Process_Table.all :=
        (others => (SF => No_SF_Id, PId => Invalid_Pid));
   ...
   function Start_Tree_Creation
     (SF                   : SF_Id;
      ...
      return                 Process_Id
   is
      Comp_Args : Argument_List (Arg_List'First .. Arg_List'Last + 12 + 1 + 2);
   ...
   begin
      ...
      if Last_Finished /= No_Process then
         Tree_Process_Table (Last_Finished) := (SF, Process);
      ...
      Tree_Process_Table (Idx) := (SF, Process);
      Last_Stored := Idx;
   end Store_Process;
   ...
   begin
      ...
      Tree_Process_Table (Idx) := (SF => No_SF_Id, PId => Invalid_Pid);
      Last_Finished := Idx;
