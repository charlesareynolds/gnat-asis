...
package ASIS_UL.Common is
   ...
   Target_From_Command_Line : String_Access;
   --  Target as it is specified by the command-line '--target=...' option.
   --  Remains equal to null if no '--target=...' option is specified.

   Target_From_Project : String_Access;
   --  Target as it is specified by the 'Target attribute in the argument
   --  project file. Remains equal to null if there is no 'Target attribute
   --  in the project file.
